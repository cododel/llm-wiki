...
export async function health(reader:Reader) {
  reader.ensureAccess();
  const rows=await reader.db<{ id:string; original:Uint8Array|null; original_hash:string|null; tags:number; relations:number; provenance:number }[]>`
    SELECT v.id,v.original,v.original_hash,
    ...
    FROM records r JOIN revisions v ON v.id=CASE WHEN ${reader.privateAccess} THEN r.current_revision ELSE r.published_revision END WHERE NOT r.archived`;
  return { visible_records:rows.length, untagged:rows.filter(r=>r.tags===0).length,
...
export async function lint(reader:Reader) {
  ...
  const [counts]=await reader.db<{ visible_records:number; empty_body:number; missing_provenance:number; drafts_without_original:number }[]>`
    SELECT count(*)::int AS visible_records,
      count(*) FILTER(WHERE length(trim(v.body))=0)::int AS empty_body,
      count(*) FILTER(WHERE r.type IN ('concept','comparison','query') AND NOT EXISTS(SELECT 1 FROM provenance p WHERE p.revision_id=v.id))::int AS missing_provenance,
      count(*) FILTER(WHERE r.type IN ('article-draft','post-draft') AND NOT EXISTS(SELECT 1 FROM provenance p WHERE p.revision_id=v.id)
